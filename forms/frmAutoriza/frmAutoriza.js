...
frmAutoriza.onshow = function () {
  ...
  // Verificar si llegamos aquí a través de una notificación push
  if (lastNotification && lastNotification.tipo === "autorizacion") {
    // Si la notificación incluye id de solicitud, obtenerla directamente
    if (lastNotification.id) {
      id = lastNotification.id;
      // Obtener detalles de esta autorización específica
      parametros = "?servicio=2b&id=" + id;
      senddata1(parametros);

      // Limpiar la notificación procesada
      lastNotification = null;
    } else {
      // Si no incluye id, verificar normalmente
